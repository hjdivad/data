...
test("A hasMany relationship has an isLoaded flag that indicates whether the ManyArray has finished loaded", function() {
  ...
  adapter.find = function(store, type, id) {
    setTimeout(async(function() {
      ...
      if (id === '3') {
        equal(get(array, 'isLoaded'), true, "After loading all records, the array isLoaded");
      } else {
        equal(get(array, 'isLoaded'), false, "After loading some records, the array isn't isLoaded");
      }
    }), 1);
  };
